...
func (proc *Processor) decodeSnapshot(
	...
	err = proc.decodeMutations(ctx, encryptedMutations, &out, validateMACs, currentState.Version, fakeIndexesToRemove, string(name))
	if err != nil {
...
func (proc *Processor) DecodePatches(
	...
	for _, patch := range list.Patches {
		...
		err = proc.decodeMutations(ctx, patch.GetMutations(), &out, validateMACs, newState.Version, fakeIndexesToRemove, string(list.Name))
		if err != nil {
			return
		}
